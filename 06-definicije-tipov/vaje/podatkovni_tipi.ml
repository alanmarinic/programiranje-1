...
let rec intbool_map intbool_map f_int f_bool ib_list = match ib_list with
       | Empty -> Empty
...
let rec intbool_separate l = 
       let rec separation_pomozna inti booli l = match l with
              | Empty -> (inti, booli)
              | Int(x, rep) -> separation_pomozna (inti @ [x]) booli rep
              | Bool(x, rep) -> separation_pomozna inti (booli @ [x]) rep
       in
       separation_pomozna [] [] l
...
| Student (student_magic, years)
       when student_magic=magic && (zahtevana_leta specialization) <= years ->
       Some wizard.name
|
